...
//declare two variable of type data that will be passed on th queue
Data DataToSend1={0x2020,1};
Data DataToSend2={0x2021,2};
/* USER CODE END PV */
...
int main(void)
{
  ...
  myQueue02Handle = osMessageQueueNew (16, sizeof(Data), &myQueue02_attributes);
...
void StartSender(void *argument)
{
  ...
  {
	  printf("Task1\n");
	  osMessagePut(meQueue01Handle,(uint32_t)&DataToSend1, 200);
	  printf("Task1 delay");
    osDelay(1000);
  }
  /* USER CODE END 5 */
...
void StartReceiver(void *argument)
{
  ...
  {
	  //printf("Task2\n");
	 retvalue =  osMessageGet(meQueue01Handle,500);
	 if(((Data*)retvalue.value.p)->source == 1)
	 {
		 printf("Received from sender1");

	 }
	 else
	 {
		 printf("Received from sender2");
	 }
...
void StartSender2(void *argument)
{
  ...
  {
	  printf("Task1\n");
	 	  osMessagePut(meQueue01Handle,(uint32_t)&DataToSend2, 200);
	 	  printf("Task2 delay");
	     osDelay(1000);
  }
